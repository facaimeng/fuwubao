...
define('common/main',['bui/menu','bui/tab'],function(require) {
  ...
  function tabNav(moduleId,tabConfig,menuConfig,collapsed,homePage){
    ...
    menu.on('menuclick',function(ev){
      var item = ev.item;  
      if(item){  
        _self.tab.addTab({id: item.get('id'), title: item.get('text'), href: item.get('href'),closeable : item.get('closeable'),path:item.get('path')},true);
      }
  ...
  BUI.augment(mainPage,{
    ...
    openPage : function(pageInfo){
      ...
      if(module){
        ...
        if(menuItem){
          ...
          tab.addTab({id: id, title: title, href: href, sourceId: sourceId,closeable: closeable,path:path,fid:fid},reload);
        }
    ...
    getFid : function(){
        var moduleId = this._getCurrentModuleId();
        module = this._getModule(moduleId);
        if(module){
            var tabItem = module.tab.getActivedItem();  
            return tabItem.get("fid");
        } 
    },
    getNav : function(){
        var moduleId = this._getCurrentModuleId();
