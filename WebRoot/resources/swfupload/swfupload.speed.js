...
if (typeof(SWFUpload) === "function") {
	...
	SWFUpload.speed.formatTime = function (baseNumber) {
		//var timeUnits = [3600, 60, 1], timeUnitLabels = ["h", "m", "s"];
		//return SWFUpload.speed.formatUnits(baseNumber, timeUnits, timeUnitLabels, false);
		var formattedStrings = [];
		var remainder = baseNumber;
		var timeUnits = [3600, 60, 1];
		var timeUnit;
			
		for (i = 0; i < timeUnits.length; i++) {
			timeUnit = timeUnits[i];
				
			unit = Math.floor(remainder / timeUnit);
			
			//unit = remainder / timeUnit;
			...
			//}
			if (unit > 0) {
				remainder = remainder % timeUnit;					
				if(unit<10){
					formattedStrings.push('0'+unit);
				}else{
					formattedStrings.push(unit);
				}				
			}
		}	
		if(formattedStrings.length==1){
			formattedStrings.unshift('00');
			formattedStrings.unshift('00');
		}else if(formattedStrings.length==2){
			formattedStrings.unshift('00');
		}else if(formattedStrings.length==0){
			formattedStrings.unshift('00');
			formattedStrings.unshift('00');
			formattedStrings.unshift('00');
		}		
	...
	SWFUpload.speed.formatBytes = function (baseNumber) {
		...
		if(baseNumber>=1073741824){
         	return (baseNumber / 1073741824).toFixed(2) + "GB";            
        }else if(baseNumber>=1048576){
        	return (baseNumber / 1048576).toFixed(2) + "MB";          
        }else if(baseNumber>11 && baseNumber<1048576){
        	return (baseNumber / 1024).toFixed(2) + "KB";            
        }else if(baseNumber>0 && baseNumber<11){
        	return baseNumber + "Byte";          
        }else{
        	return "0KB";  
        }	
	};
